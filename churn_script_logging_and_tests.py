# -*- coding: utf-8 -*-
"""Test Script for churn customer analysis.
...
def test_eda(perform_eda: Callable) -> None:
    ...
    try:
        ...
        logging.error(
            f"Testing perform_eda: Column {err.args[0]} not" f" found in input schema."
        )
        raise err
...
def test_encoder_helper(encoder_helper: Callable) -> None:
    ...
    bank_data = encoder_helper(
        bank_data=bank_data, category_lst=category_lst, response="Churn"
    )

    for col in category_lst:

        bank_data_test = bank_data.groupby(by=col).agg(
            test_col=("Churn", "mean"), ref_val=(col + "_" + "Churn", "mean")
        )

        try:
            assert all(bank_data_test["test_col"] == bank_data_test["ref_val"])
            logging.info(
                f"Testing test_encoder_helper: SUCCESS " f"for '{col}' column."
            )
...
def test_perform_feature_engineering(
        perform_feature_engineering: Callable) -> None:
    """
    ...
    X_train, X_test, y_train, y_test = perform_feature_engineering(
        bank_data=bank_data, response="Churn"
    )
    ...
    for key, val in validation_dict.items():
        ...
        try:
            assert isclose(
                ...
                abs_tol=1)
            logging.info(
                f"Testing test_perform_feature_engineering: SUCCESS "
...
def test_train_models(train_models: Callable) -> None:
    ...
    for model_file in model_files:
        ...
        model = load_model(model_name=model_file.split(".")[0])
        ...
        try:
            assert score > 0.5
            logging.info(
                f"Testing train_models: SUCCESS {model_file} has "
